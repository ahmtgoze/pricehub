...
export default function AdvantageProductTag() {
  ...
  const calculateProfit = (price, commissionRate, item) => {
    try {
      ...
      let { shippingCost, shippingVatRate, baremUsed } = promosyonKargosu({ platform: platformObj, urun: matchedProduct, fiyat: price, tarifeler: platformShippingRates, kurallar: ciftKargoKurallari(settings) });
  ...
  // KOMİSYON TARİFESİ "Var" olan urunun komisyonu, kendi tarife
  // kademelerinden gelir: yildiz fiyati tarifenin hangi aralagina duserse
  // o aralikin komisyonu uygulanir. Urun etiket programindan CIKARILMAZ ve
  // bu, Komisyon Tarifesi sayfasinda ne secildiginden bagimsizdir.
  ...
  const getDynamicCommissionForPrice = (item, price) => {
    // Tarife "Var" ise komisyon, urunun kendi tarife kademesinden gelir:
    // yildiz fiyati tarifenin hangi araligina duserse o aralikin komisyonu.
    //
    // Onceki surum ilk buldugu kaydi aliyordu; KPBŞ1'in dort kaydi var ve
  ...
  const handleSmartAutoSelect = () => {
    ...
    const updated = uploadedData.map(item => {
      ...
      // Hicbir aralik hedefi tutmadi -> SECILMEZ. (15 Eylul aksami: 'en karli
      // barem onerisini manuel sec' geri donusu kaldirildi; 18 urunu 349,99'a
      // cekmisti, Plus'ta zarara donuyordu. Kullanici: 'ben secmedim ki'.)
      skippedTargetNotMet++;
  ...
  const secimleriKaldirVeKaydet = async () => {
    const temiz = uploadedData.map((item) => ({ ...item, selected_range: 'none', selected_price: 0, manual_price: 0 }));
    ...
    try {
      ...
      for (let i = 0; i < kayitli.length; i += 30) {
        ...
        await Promise.all(b.map((it) => db.entities.AdvantageProductTag.update(it.id, { selected_range: 'none', selected_price: 0, manual_price: 0 })));
        if (i + 30 < kayitli.length) await new Promise((r) => setTimeout(r, 150));
      }
      queryClient.invalidateQueries({ queryKey: ['advantageProductTags'] });
