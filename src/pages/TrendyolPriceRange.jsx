...
export default function TrendyolPriceRange() {
  ...
  const handlePriceSelect = (index, rangeType, price) => {
    ...
    if (updated[index].selected_range === rangeType) {
      ...
      // Secim kalkinca urun hicbir pencereye ait degil.
      updated[index].secim_penceresi = null;
    } else {
      updated[index].selected_range = rangeType;
      updated[index].selected_price = price;
      // Secim O ANDA acik olan pencereye aittir. Bir urun ayni anda iki
      // pencerede olamaz: dosyada urun basina tek satir ve tek "Tarife
      // Secimi" hucresi var. Baska pencerede seciliyse buraya TASINIR.
      updated[index].secim_penceresi = secilenPencere || null;
  ...
  const handleSmartAutoSelect = () => {
    ...
    const updated = uploadedData.map(item => {
      ...
      for (const range of ranges) {
        ...
        if (meetsTarget) {
          selectedCount++;
          return { ...item, selected_range: `range_${range.rangeNum}`, selected_price: range.price, secim_penceresi: secilenPencere || null };
        }
      }
              ...
              {pencereSecenekleri.length > 1 && (
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    ...
                    işaretli kalmaz. Hepsi <strong>tek Excel'e</strong> kendi tarifesiyle yazılır.
                  </p>
                  {seciliKirilim && (
                      ...
                      {sortedData.map((item, index) => {
                        ...
                        return (
                          <tr key={index} className="border-b hover:bg-secondary">
                            ...
                            <td className="p-3">
                              {matchedProduct ? (
                                <div className="text-center text-xs"><div className="font-medium text-muted-foreground">{matchedProduct.category_name}</div></div>
                            ...
                            <td className="p-3">
                              {(() => {
                                ...
                                if (selectedPrice === 0 || item.selected_range === 'none') return <div className="text-center text-muted-foreground/70 text-xs">-</div>;
                                ...
                                if (!bestBaremSuggestion) return <div className="text-center text-muted-foreground/70 text-xs">-</div>;
