...
import {
  Package, Percent, Store, Truck, TrendingUp, TrendingDown,
  AlertCircle, CheckCircle2, BarChart2, ArrowUpDown, Tag, ChevronDown, ChevronUp
} from 'lucide-react';
...
const { data: platforms = [] } = useQuery({
  ...
  queryFn: async () => {
    ...
    // ✅ platform_type'a göre tekilleştir
  ...
  // ✅ Platform bazlı listelenmemiş ürünler
  const unlistedByPlatform = useMemo(() => {
    ...
    return allPlatformTypes.map(platformType => {
      // Bu platform tipinde eşleştirilmiş ürünlerin product_id'lerini bul
  ...
  const profitDistribution = useMemo(() => {
    if (filteredByRange) {
      ...
      const negativeValues = filteredPrices.filter(p => (p.profit_rate ?? 0) < 0);
      const positiveValues = filteredPrices.filter(p => (p.profit_rate ?? 0) >= 0);
      const result = [];
      if (min < 0) result.push({ name: `${customMinProfit}–0%`, value: negativeValues.length, type: 'negative' });
      if (max > 0) result.push({ name: `0–${customMaxProfit}%`, value: positiveValues.length, type: 'positive' });
      return result;
    }
    const buckets = {
      '< 0%': 0, '0–10%': 0, '10–20%': 0, '20–30%': 0,
      '30–40%': 0, '40–50%': 0, '50–75%': 0, '75–100%': 0,
      '100–200%': 0, '200–300%': 0, '> 300%': 0
    };
    ...
    return activePlatforms.map(platform => {
      ...
      if (prices.length > 0) {
        ...
        const totalCost = products
          .filter(prod => prices.some(pp => pp.product_id === prod.id))
          .reduce((s, prod) => s + (prod.cost || 0), 0);
  ...
  const handleShowProducts = () => {
    ...
    const filtered = products
      .filter(p => p.created_date && new Date(p.created_date) >= start && new Date(p.created_date) <= end)
  ...
  const platformColors = {
    trendyol: 'border-orange-200 bg-orange-50',
    hepsiburada: 'border-yellow-200 bg-yellow-50',
    website: 'border-indigo-200 bg-indigo-50',
  };
  const platformTextColors = {
    trendyol: 'text-orange-700',
    hepsiburada: 'text-yellow-700',
    website: 'text-indigo-700',
  };
  const platformBadgeColors = {
    trendyol: 'bg-orange-100 text-orange-800',
    hepsiburada: 'bg-yellow-100 text-yellow-800',
    ...
    <div className="min-h-screen">
      <div className="max-w-[1600px] mx-auto px-3 sm:px-6 py-5 sm:py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 mb-6">
          ...
          <StatCard icon={AlertCircle} label="Fiyatlanmamış" value={unpricedProducts.length} color={unpricedProducts.length > 0 ? 'red' : 'green'} />
        </div>
                  ...
                  <div key={platform.id} className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {negativeProfitCount > 0 ? (
                        <Badge variant="destructive" className="text-xs">{negativeProfitCount} negatif</Badge>
                      ) : (
                        <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                      )}
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-xs text-center">
                      <div><p className="text-gray-400 mb-0.5">Ort. Kâr</p><p className={`font-bold ${profitColor(avgProfit)}`}>{avgProfit !== null ? formatTurkishPercent(avgProfit) : '—'}</p></div>
                      <div><p className="text-gray-400 mb-0.5">Min</p><p className={`font-medium ${profitColor(minProfit)}`}>{minProfit !== null ? formatTurkishPercent(minProfit) : '—'}</p></div>
                      <div><p className="text-gray-400 mb-0.5">Maks</p><p className={`font-medium ${profitColor(maxProfit)}`}>{maxProfit !== null ? formatTurkishPercent(maxProfit) : '—'}</p></div>
                    </div>
                    <p className="text-xs text-gray-400 mt-2 text-right">{prices} fiyat</p>
                  </div>
                ))}
              </div>
                    ...
                    {platformSummary.map(({ platform, prices, avgProfit, minProfit, maxProfit, negativeProfitCount }) => (
                      <tr key={platform.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="py-2.5 text-center">
                          {negativeProfitCount > 0 ? (
                            <Badge variant="destructive" className="text-xs">{negativeProfitCount}</Badge>
                          ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-end w-full">
              ...
              <Button onClick={handleShowProducts} disabled={!startDate || !endDate} className="w-full sm:w-auto shrink-0">
                Listele
              </Button>
            </div>
            {showProductsList && (
                ...
                <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                  {newProductsList.length > 0 ? newProductsList.map(product => (
                    <div key={product.id} className="flex items-center justify-between px-3 py-2.5 bg-gray-50 rounded-xl border border-gray-100 hover:bg-gray-100 transition-colors">
                      ...
                      <span className="text-xs text-gray-500 shrink-0 ml-3 whitespace-nowrap">
                        {new Date(product.created_date).toLocaleDateString('tr-TR')}
                      </span>
                    </div>
                  )) : (
          ...
          {/* ✅ Platformda Listelenmeyen Ürünler */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
            ...
            <div className="space-y-3">
              {unlistedByPlatform.map(({ platformType, name, listedCount, unlistedCount, unlistedProducts }) => (
                <div key={platformType} className={`rounded-xl border overflow-hidden ${platformColors[platformType]}`}>
                  <button
                    onClick={() => setExpandedPlatform(expandedPlatform === platformType ? null : platformType)}
                    className="w-full flex items-center justify-between px-4 py-3"
                  >
                    <div className="flex items-center gap-3">
                      <span className={`font-semibold text-sm ${platformTextColors[platformType]}`}>{name}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${platformBadgeColors[platformType]}`}>
                        {listedCount} listelendi
                      </span>
                      {unlistedCount > 0 && (
                        <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-red-100 text-red-700">
                          {unlistedCount} listelenmemiş
                        </span>
                      )}
                      {unlistedCount === 0 && (
                        <CheckCircle2 className="h-4 w-4 text-emerald-500" />
