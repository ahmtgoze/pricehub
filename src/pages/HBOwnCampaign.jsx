...
export default function HBOwnCampaign() {
  ...
  function calculateProfit(price, commissionRate, product) {
    try {
      ...
      let { shippingCost, shippingVatRate, baremUsed } = promosyonKargosu({ platform: platform, urun: product, fiyat: price, tarifeler: platformShippingRates, kurallar: ciftKargoKurallari(settings) });
  ...
  const openDetail = (price, commissionRate, product) => {
    ...
    setDetailModal({
      ...
      calculationDetails: { productCost: gecerliMaliyet(product), productVatRate: product?.vat_rate || 20, commissionRate, packagingCost: calc.breakdown?.packagingCost || 0, printingCost: product?.printing_cost || 0, extraCost: product?.extra_cost || 0, shippingCost: calc.breakdown?.shippingCost || 0, posServiceFee: calc.breakdown?.posServiceFee || 0 },
    });
  };
